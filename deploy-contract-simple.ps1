...
if (-not (Test-Command "soroban")) {
    Write-Host "[INFO] Soroban CLI not found. Installing..." -ForegroundColor Yellow
    try {
        cargo install --locked soroban-cli
        Write-Host "[OK] Soroban CLI installed" -ForegroundColor Green
    }
    catch {
    ...
    Write-Host "[OK] Soroban CLI found" -ForegroundColor Green
}
...
try {
    ...
    Write-Host "[OK] WASM target added" -ForegroundColor Green
}
catch {
    Write-Host "[WARN] WASM target might already be installed" -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Key '$KeyName' already exists: $existingKey" -ForegroundColor Green
    }
    else {
        soroban keys generate $KeyName
        $newKey = soroban keys address $KeyName
        Write-Host "[OK] Generated new key '$KeyName': $newKey" -ForegroundColor Green
    }
}
catch {
...
try {
    ...
    Write-Host "[OK] Network '$Network' configured and selected" -ForegroundColor Green
}
catch {
    Write-Host "[WARN] Network might already be configured" -ForegroundColor Yellow
}
...
if ($Network -eq "testnet") {
    ...
    try {
        ...
        Write-Host "[OK] Account funded with testnet XLM" -ForegroundColor Green
    }
    catch {
        Write-Host "[WARN] Account funding might have failed or account already has funds" -ForegroundColor Yellow
    }
}
...
try {
    ...
    Write-Host "[OK] Contract ID saved to CONTRACT_ID.txt" -ForegroundColor Green
}
catch {
...
try {
    ...
    Write-Host "[WARN] Could not retrieve account information" -ForegroundColor Yellow
}
...
Write-Host "`nTesting contract..." -ForegroundColor Blue
try {
    Write-Host "Running basic contract test..." -ForegroundColor Yellow
    # Test whitelist function
    $testResult = soroban contract invoke --id $contractId --source $KeyName --network $Network -- is_whitelisted --user $publicKey 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Contract is responding to function calls" -ForegroundColor Green
        Write-Host "Whitelist status: $testResult" -ForegroundColor Cyan
    }
    else {
        Write-Host "[WARN] Contract test failed, but deployment was successful" -ForegroundColor Yellow
    }
}
catch {
    Write-Host "[WARN] Could not test contract, but deployment was successful" -ForegroundColor Yellow
}

Write-Host "`nNext steps:" -ForegroundColor Blue
Write-Host "1. Install Visual Studio Build Tools if not already installed" -ForegroundColor White
Write-Host "2. Update the frontend with the new contract ID" -ForegroundColor White
Write-Host "3. Test transfers using Freighter wallet" -ForegroundColor White
